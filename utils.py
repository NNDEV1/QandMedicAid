...
def rho_select(pad, lengths):
    ...
    idx_ = (lengths - 1).view(-1, 1).expand(pad.size(0),
                                            pad.size(2)).unsqueeze(1)
        ...
        if args.use_RAD:
            if (
                os.path.isfile(os.path.join(
                    args.RAD_dir, "embed_tfidf_weights.pkl"))
                == True
            ):
                ...
                tfidf, weights = dataset_RAD.tfidf_from_questions(
                    ["train"], None, dict)
                w_emb.init_embedding(
                    os.path.join(
                        args.RAD_dir, "glove6b_init_300d.npy"), tfidf, weights
                )
                with open(
