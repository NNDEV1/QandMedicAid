...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.autoencoder:
            ae_images_data = self.ae_images_data[entry["image"]].reshape(
                128 * 128)
...
def tfidf_from_questions(names, args, dictionary, dataroot="data", target=["rad"]):
    ...
    tfidf = torch.sparse.FloatTensor(
        torch.LongTensor(inds), torch.FloatTensor(vals))
    ...
    print(
        "tf-idf stochastic matrix (%d x %d) is generated."
        % (tfidf.size(0), tfidf.size(1))
    )
...
if __name__ == "__main__":
    ...
    dictionary = Dictionary.load_from_file("data_RAD/dictionary.pkl")
    w_emb = WordEmbedding(dictionary.ntoken, 300, 0.0, "c")
